...
class RegPattern(object):
	...
	'''
	# re_time = r"([0-2])?([0-9])?\s?:3([0-9])?|[0-2]?[0-9]|(a|A|p|P)\s?\.?(m|M)?\.?"
	# re_time = r"[0-2]?[0-9]?\s?:3[0-9]"
	#first check the re_time and then the words
	re_all = r"[0-2]?[0-9]?\s?:3[0-9]|[\w']+|['.,!?;]{2,20}|['.,!?;]|<PM>|<USER>|<HASHTAG>|:-\)|:\)|:D|:o\)|:\]|:3|:c\)|:>|=\]|8\)|=\)|:\}|:\^\)|:-\)\)|:‑D|8‑D|8D|x‑D|xD|X‑D|XD|=‑D|=D|=‑3|=3|B\^D|>:\[|:‑\(|:\(|:‑c|:c|:‑<|:<|:‑\[|:\[|:\{|;\(|D:<|D:|D8|D;|D=|DX|v\.v|D‑':|>:O|:‑O|:O|:‑o|:o|8‑0|O_O|o‑o|O_o|o_O|o_o|O-O|:\*|:-\*|:\^\*|\( '\}\{'\)"
	...
	def get_pattern(self, text, re_text):
		...
		elif re_text == 're_time':
			return self.__get_time(text)
	...
	def __get_time(self, text):
		match = []
		p = re.compile(self.re_time)
		match = p.findall(text)
		return match
